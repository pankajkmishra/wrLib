...
#include <cmath>
#include <iostream>
#include </home/pkmishra/PNP/Eigen/Core>
#include </home/pkmishra/PNP/Eigen/StdVector>
 ...
 * @param is_inside Charasteristic function of the domain, which takes a point and returns `true` if
 * and only if given point is inside the domain.
 * @param h Function perscribing local nodal spacing, which takes a point and returns a positive
 ...
 * is ignored in 1-D.
 * @param max_points Maximal number of points. The algorithm terminates once thi threshold has been
 * reached.
 * @param generator (Pseuod) random generator to be used for randomization.
 */
...
std::vector<Eigen::Matrix<scalar_t, dim, 1>,
    ...
    while (cur_node < end_node && cur_node < max_points) {
